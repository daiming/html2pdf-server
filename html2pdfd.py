...
SUPPORTED_TYPES = {
    'application/pdf': lambda html, buffer: html.write_pdf(buffer,presentational_hints=True),
    'image/png': lambda html, buffer: html.write_png(buffer),
    'image/jpeg': lambda html, buffer: render_to_jpeg(html, buffer),
}

MAX_HTML_SIZE = 1024 * 1024 * 500  # 500MiB
...
def app(request: Request) -> Response:
    ...
    matched = request.accept_mimetypes.best_match(
        supported_types, default='application/pdf')
    if not matched:
        return Response(
            json.dumps({
                'error':
                'not-acceptable',
                'message':
                'unsupported type; the list of supported '
...
def main():
    ...
    parser.add_argument(
        '--host', '-H', default='0.0.0.0', help='host to listen [%(default)s]')
    parser.add_argument(
        '--port',
        '-p',
        type=int,
    ...
    parser.add_argument(
        '--pong-path', help='pong path to respond to to ping (e.g. /pong/)')
    parser.add_argument(
